...
- *Raw-predecessor impossibility check:* when the raw predecessor in the sorted array was itself dropped (e.g. it was a ghost fix), `kept[-1]` may be months or years earlier. That makes the elapsed time enormous, so a geographically remote ghost can appear reachable at 20 knots and slip through. After the main impossible-distance check, if the raw predecessor differs from the last kept point, a second impossibility check is run against the raw predecessor's position and timestamp. This closes the gap where a chain of dropped ghosts causes the filter to lose spatial context.
