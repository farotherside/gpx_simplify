...
- *Dropped-predecessor impossibility check:* ghost clusters often contain multiple consecutive points all at the same remote location, each a few seconds apart. For each such point, `kept[-1]` (the last real fix) may be months or years earlier — making the normal elapsed-time budget enormous — while the raw predecessor `points[i-1]` was itself a ghost that was just dropped. When that predecessor was dropped *and* the normal time budget exceeds 500 km (i.e. `kept[-1]` is genuinely stale), the filter uses `points[i-1].time` as a tight time reference (~seconds) while keeping `kept[-1]`'s position as the location anchor. A real boat cannot travel 3000+ km from the last known position in those few seconds, so the ghost is correctly dropped. The 500 km budget guard ensures that legitimate points after short GPS dropouts (where the budget is modest, e.g. 74 km after a 2-hour gap) are not affected.
