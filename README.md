...
- *Cross-context impossibility check:* ghost clusters often contain multiple consecutive points all at the same remote location, each a few seconds apart. For each such point, `kept[-1]` (the last real fix) may be months or years earlier — making `max_plausible_m` enormous — while the raw predecessor is another ghost at the same spot, so the distance to it is near zero. Both standard checks pass. The cross-context check closes this gap: it tests whether the distance from `kept[-1]`'s *position* to the current point is impossible given the *minimum* elapsed time seen in the raw array (i.e. the few seconds since the previous raw point). For a ghost cluster that elapsed time is tiny (~5 s), the distance budget is tiny (~50 m), and the 3000+ km distance to the ghost location triggers an immediate drop.
